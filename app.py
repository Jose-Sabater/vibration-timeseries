...
app.layout = html.Div(
    children=[
        ...
        """
                ...
                html.P(
                    children="Please set how many standard deviations to use as control limit"
                ),
                dcc.Input(
                    id="stdtolerance",
                    type="number",
        ...
        html.Div(
            [
                ...
                html.P(
                    children=f"Please select the size you would like for training (recommended is {round(0.85*(len(df)))})"
                ),
                dcc.Input(
                    id="train_size_input",
                    type="number",
                    ...
                    max=len(df) * 0.98,
                    step=1,
                    value=round(len(df) * 0.8),
                ),
                dcc.Graph(id="train_test_graph"),
                html.P(
                    children="Select the type of forecast you would like to use. Autoregressive integrated moving average or seasonal"
                ),
                dcc.Dropdown(
                    id="forecast-model",
                ...
                html.H4(
                    "Please introduce the amount of values you want to predict:",
                    style={"display": "inline-block", "margin-right": 20},
                ),
                dcc.Input(
                    id="predictions",
...
def populate_graph(dummy):
    pie_graph = px.pie(
        df_raw,
        names="onoff",
        title="Uptime of the machinery",
        color_discrete_sequence=[scania_blue1, "#B0B7C4"],
    )
    pie_graph.update_layout(
        plot_bgcolor=scania_white,
        paper_bgcolor=scania_white,
        font_family="Scania Sans",
    )
    return pie_graph
...
def display_graph(graphtype, df_sliced):
    ...
    if graphtype == "timeseries":
        ...
        fig.add_hline(
            y=16, line_width=1, line_dash="dash", line_color="#D6001C"
        )
        fig.add_hline(
            y=13, line_width=1, line_dash="dash", line_color="#D6001C"
        )
        fig.update_layout(
            plot_bgcolor=scania_white,
            paper_bgcolor=scania_white,
...
def display_tolerances(stdtolerance):
    ...
    rolmean_plot = go.Scatter(
        x=filtered_df.index, y=rolmean, name="rolmean", line_color=scania_green
    )
    df_plot = go.Scatter(
        x=filtered_df.index,
    ...
    lower_bound_plot = go.Scatter(
        x=filtered_df.index, y=lower_bound, name="LCL", line_color="#D6001C"
    )
...
def train_test(df_size, p, d, q, model, prediction_size):
    ...
    fig = px.line(
        acceleration,
        color="label",
        color_discrete_sequence=[scania_blue1, scania_red],
    )
    ...
    if model == "Arima":
        ...
        end_time = time.time()
        model_final_prediction = ARIMA(
            acceleration.iloc[:, 0], order=(p, d, q)
        )
    ...
    prediction_df["Predictions"] = results_model.predict(
    ...
    rmse = np.sqrt(
        mean_squared_error(df_test.values, prediction_df["Predictions"])
    )
    print("RMSE: ", rmse)
    ...
    test_plot = go.Scatter(
        x=df_test.index, y=df_test, name="test data", line_color=scania_red
    )
    ...
    prediction_df_1["predictions"] = results_final_prediction.predict(
        start=prediction_df_1.index[0], end=prediction_df_1.index[-1]
    )
    prediction_df_1.index = pd.RangeIndex(
        start=acceleration.index[-1],
        stop=acceleration.index[-1] + prediction_size,
        step=1,
    )
    ...
    forecast_overlay.update_layout(
        plot_bgcolor=scania_white,
        paper_bgcolor=scania_white,
        font_family="Scania Sans",
    )
    ...
    return (
        ...
        fig2,
        forecast_overlay,
        html.H5(f"Your RMSE value is: {rmse}"),
        prediction_plot,
    )
